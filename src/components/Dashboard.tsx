...
export default function Dashboard() {
  ...
  // NOTE: Do not auto-create debts/entries here. Data should only change from user input.

  const lockApp = () => {
    const uid = user?.uid || auth.currentUser?.uid;
    if (!uid) return;
    localStorage.setItem('aelyn.locked.uid', uid);
  ...
  const [activeTab, setActiveTab] = useState<
    'overview' | 'ledger' | 'debts' | 'investments' | 'stakeholders' | 'reports' | 'admin' | 'logs'
  >('overview');
  const notifTrayRef = useRef<HTMLDivElement | null>(null);
  ...
  // Auto-close notification tray when clicking outside
  useEffect(() => {
    if (!showNotifTray) return;
    const onDown = (e: MouseEvent) => {
      const tray = notifTrayRef.current;
      if (!tray) return;
      const target = e.target as Node | null;
      if (target && tray.contains(target)) return;
  ...
  const stakeholderList = uniqueUsers
    .filter((u) => u.name && !u.name.toLowerCase().includes('admin'))
    .slice(0, 2);
    ...
    <div className="min-h-screen aelyn-bg flex font-sans text-slate-200 selection:bg-gold-light">
      ...
      <aside className="w-[280px] hidden lg:flex flex-col fixed inset-y-0 left-0 z-50 px-6 py-8">
        ...
        <nav className="mt-6 aelyn-panel-soft rounded-2xl p-3 flex flex-col gap-1">
          {[
            { id: 'overview', label: 'Dashboard', icon: PieChartIcon, onClick: () => goToSection('section-dashboard') },
            {
              id: 'ledger',
              label: 'Transactions',
              icon: Receipt,
            ...
            { id: 'investments', label: 'Investments', icon: Landmark, onClick: () => setActiveTab('investments') },
            ...
            { id: 'stakeholders', label: 'Stakeholders', icon: Users, onClick: () => setActiveTab('stakeholders') },
            ...(profile?.role === 'admin' || profile?.isDeveloperMode
              ? [{ id: 'admin', label: 'Admin Console', icon: Settings, onClick: () => setActiveTab('admin') } as const]
              : []),
        ...
        <div className="mt-auto aelyn-panel-soft rounded-2xl p-3 flex items-center justify-between gap-3">
          ...
          <button
            onClick={lockApp}
            className="w-10 h-10 rounded-xl flex items-center justify-center text-slate-400 hover:text-sky hover:bg-white/5 transition-all"
            type="button"
            aria-label="Lock app"
            title="Lock app"
          >
      ...
      <div className="flex-grow lg:ml-[280px] flex flex-col">
        <header className="sticky top-0 z-40 px-6 lg:px-10 py-6">
          <div className="aelyn-panel-soft rounded-2xl px-6 py-4 flex items-center justify-between gap-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                onClick={toggle}
                className="p-2.5 bg-white/5 border border-white/10 rounded-xl text-slate-300 hover:text-white transition-all"
                type="button"
              ...
              {/* Mobile/Tablet profile + signout (sidebar is hidden <lg) */}
              <div className="lg:hidden flex items-center gap-2">
                <button
                  onClick={() => setShowProfileModal(true)}
                  className="w-10 h-10 rounded-xl overflow-hidden border border-white/10 bg-white/5 shrink-0 hover:bg-white/10 transition-all"
                  type="button"
                ...
                <button
                  onClick={signOut}
                  className="w-10 h-10 rounded-xl flex items-center justify-center text-slate-300 bg-white/5 border border-white/10 hover:text-danger hover:bg-danger/10 transition-all"
                  type="button"
                  aria-label="Sign out"
                  title="Sign out"
                >
                ...
                <button
                  onClick={lockApp}
                  className="w-10 h-10 rounded-xl flex items-center justify-center text-slate-300 bg-white/5 border border-white/10 hover:text-sky hover:bg-white/10 transition-all"
                  type="button"
                  aria-label="Lock app"
                  title="Lock app"
                >
        ...
        {/* Dashboard Grid View */}
        <main className="px-4 sm:px-6 lg:px-10 pb-28 lg:pb-20 flex-grow" id="section-dashboard">
          ...
          {/* Filter Controls */}
          <div className="aelyn-panel-soft p-6 rounded-2xl border border-white/10 flex flex-wrap items-center gap-6 mb-12" id="section-reports">
          ...
          <AnimatePresence mode="wait">
            {activeTab === 'overview' && (
              <div className="flex flex-col gap-10">
                ...
                <motion.div 
                  ...
                  className="aelyn-panel rounded-[32px] sm:rounded-[40px] p-6 sm:p-12 relative overflow-hidden"
                >
                  ...
                  <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-12">
                    <div className="flex flex-col gap-4 max-w-lg">
                      <div className="flex items-center gap-3">
                        ...
                        <p className="text-[10px] font-bold uppercase tracking-[0.4em]" style={{ color: 'var(--aelyn-fg)' }}>
                          Institutional Vitality Index
                        </p>
                      </div>
                      <h2 className="text-4xl md:text-5xl font-bold tracking-tighter leading-tight" style={{ color: 'var(--aelyn-fg)' }}>
                        Financial Health Performance Score
                      </h2>
                      <p className="text-sm leading-relaxed font-medium" style={{ color: 'var(--aelyn-muted)' }}>
                        A composite metric evaluating your revenue trajectory, liability exposure, and operational margins. This score dictates your current institutional borrowing capacity.
                      </p>
                    </div>
                    ...
                    <div className="flex flex-col items-center">
                      <div className="relative w-48 h-48 flex items-center justify-center">
                        <svg className="w-full h-full -rotate-90">
                          <circle cx="96" cy="96" r="88" fill="transparent" stroke="rgba(148,163,184,0.25)" strokeWidth="12" />
                          <motion.circle 
                            cx="96" cy="96" r="88" 
                            fill="transparent" 
                            stroke="currentColor" 
                        ...
                        <div className="absolute inset-0 flex flex-col items-center justify-center">
                          ...
                          <span className="text-[10px] font-bold uppercase tracking-widest mt-1" style={{ color: 'var(--aelyn-muted)' }}>
                            PERCENTILE
                          </span>
                        </div>
                      </div>
                      <p className="mt-6 text-[10px] font-bold uppercase tracking-[0.2em] px-6 py-2 bg-white/5 rounded-full border border-white/10" style={{ color: 'var(--aelyn-muted)' }}>
                        ...
                        filteredDebts.map((debt) => (
                          <div key={debt.id} className="px-5 py-4 flex items-center justify-between gap-4">
                            ...
                            <div className="flex items-center gap-3 shrink-0">
                              <div className={cn(
                                "text-[14px] font-semibold font-mono tracking-tight",
                                debt.status === 'Unpaid' ? "text-danger" : "text-slate-200"
                              )}>
            ...
            {activeTab === 'investments' && (
              ...
              >
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                  <Card title="Investment Fund" badge="40% Allocation">
                    <div className="space-y-4">
                      <p className="text-[12px] text-slate-500">
                        This is calculated from net profit for the selected date range.
                      </p>
                      <div className="text-3xl font-semibold tracking-tight font-mono text-slate-200">
                        {formatCurrency(investmentFund, userCurrency)}
                      </div>
                      <div className="flex items-center justify-between text-[12px] text-slate-500">
                        <span>Net profit</span>
                        <span className="font-mono text-slate-200">{formatCurrency(netProfit, userCurrency)}</span>
                      </div>
                    </div>
                  </Card>

                  <Card title="Distribution Summary" badge="60% Stakeholders">
                    <div className="space-y-4">
                      <div className="flex items-center justify-between">
                        <span className="text-[12px] text-slate-500">Stakeholder pool</span>
                        <span className="text-[14px] font-semibold font-mono text-slate-200">
                          {formatCurrency(stakeholderDistribution, userCurrency)}
                        </span>
                      </div>
                      <div className="divide-y divide-white/10 border border-white/10 rounded-2xl overflow-hidden bg-white/5">
                        {(stakeholderList.length ? stakeholderList.map((s) => s.name) : ['Stakeholder A', 'Stakeholder B']).map((name) => (
                          <div key={name} className="px-5 py-4 flex items-center justify-between gap-4">
                            ...
                            <div className="text-[14px] font-semibold font-mono text-slate-200">
                              {formatCurrency(individualShare, userCurrency)}
                            </div>
                          </div>
                        ))}
                      </div>
            ...
            {activeTab === 'stakeholders' && (
              ...
              >
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                  <Card title="Stakeholder Allocation" badge="Distribution">
                    <div className="space-y-4">
                      <div className="text-[12px] text-slate-500">
                        Distribution (60%): <span className="text-slate-200 font-semibold">{formatCurrency(stakeholderDistribution, userCurrency)}</span>
                      </div>
                      <div className="divide-y divide-white/10 border border-white/10 rounded-2xl overflow-hidden bg-white/5">
                        {(stakeholderList.length ? stakeholderList.map((s) => s.name) : ['Stakeholder A', 'Stakeholder B']).map((name, idx) => (
                          <div key={name} className="px-5 py-4 flex items-center justify-between gap-4">
                            <div className="min-w-0">
                              <div className="text-[13px] font-medium text-white truncate">{name}</div>
                              <div className="text-[12px] text-slate-500">{idx === 0 ? '30%' : '30%'}</div>
                            </div>
                  ...
                  <Card title="Contributors" badge={`${uniqueUsers.length} Users`}>
                    <div className="divide-y divide-white/10 border border-white/10 rounded-2xl overflow-hidden bg-white/5">
                      {uniqueUsers.length === 0 ? (
                        <div className="p-10 text-center text-slate-500 text-sm">No contributors yet.</div>
                      ) : (
                        uniqueUsers.map((u) => (
                          <div key={u.id} className="px-5 py-4 flex items-center justify-between gap-4">
                            <div className="flex items-center gap-3 min-w-0">
                              <div className="w-10 h-10 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center text-[12px] font-semibold text-slate-200">
                                {u.name?.charAt(0)?.toUpperCase() || 'U'}
                              </div>
                              <div className="min-w-0">
                                <div className="text-[13px] font-medium text-white truncate">{u.name}</div>
                                <div className="text-[12px] text-slate-500">User ID: {u.id.slice(0, 8)}</div>
                              </div>
                            </div>
                          </div>
                        ))
            ...
            {activeTab === 'reports' && (
              ...
              >
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                  <Card title="Exports" badge="Reports">
                    <div className="space-y-4">
                      <p className="text-[12px] text-slate-500">
                        Export current filtered data (date range + type) to CSV or PDF.
                      </p>
                      <div className="flex gap-3">
                        <button
                          onClick={exportCSV}
                          className="flex-1 bg-white/5 border border-white/10 text-slate-200 text-[12px] font-medium px-4 py-3 rounded-xl hover:bg-white/10 transition-all"
                          type="button"
                        >
                          Export CSV
                        </button>
                        <button
                          onClick={exportPDF}
                          className="flex-1 bg-gradient-to-r from-sky to-sky-2 text-navy text-[12px] font-semibold px-4 py-3 rounded-xl hover:brightness-110 active:brightness-95 transition-all shadow-[0_18px_50px_rgba(46,229,210,0.15)]"
                          type="button"
                        >
                          Generate PDF
                        </button>
                  ...
                  <Card title="Quick Summary" badge="Current Range">
                    <div className="space-y-4">
                      <div className="flex items-center justify-between">
                        <span className="text-[12px] text-slate-500">Total revenue</span>
                        <span className="text-[14px] font-semibold font-mono text-slate-200">{formatCurrency(totalRevenue, userCurrency)}</span>
                      </div>
                      <div className="flex items-center justify-between">
                        <span className="text-[12px] text-slate-500">Total expenses</span>
                        <span className="text-[14px] font-semibold font-mono text-slate-200">{formatCurrency(totalExpenses, userCurrency)}</span>
                      </div>
                      <div className="flex items-center justify-between">
                        <span className="text-[12px] text-slate-500">Net profit</span>
                        <span className="text-[14px] font-semibold font-mono text-slate-200">{formatCurrency(netProfit, userCurrency)}</span>
                      </div>
                    </div>
                  </Card>
                </div>
      ...
      {/* Mobile Bottom Nav */}
      <nav className="lg:hidden fixed bottom-0 inset-x-0 z-50 px-4 pb-[max(12px,env(safe-area-inset-bottom))] pt-3">
        <div className="aelyn-panel-soft rounded-2xl border border-white/10 px-3 py-2 flex items-center justify-between">
          {[
            { id: 'overview', label: 'Home', icon: LayoutDashboard },
            { id: 'ledger', label: 'Txns', icon: Receipt },
            { id: 'debts', label: 'Debts', icon: Wallet },
            { id: 'reports', label: 'Reports', icon: BarChart3 },
            { id: 'stakeholders', label: 'People', icon: Users },
          ]
            .filter((i) => (i.id === 'stakeholders' ? true : true))
            .map((item) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    selected ? 'bg-white/10 text-white' : 'text-slate-400 hover:bg-white/5 hover:text-slate-200',
                  )}
                >
                  <item.icon size={18} className={cn(selected ? 'text-sky' : '')} />
      ...
      {/* Notification Tray */}
      {showNotifTray && (
        <div ref={notifTrayRef} className="fixed top-24 right-6 lg:right-10 w-[360px] aelyn-panel rounded-2xl z-[60] overflow-hidden">
...
const ProfileModal = ({ onClose, profile }: { onClose: () => void, profile: any }) => {
  ...
  const handleToggleBiometrics = async () => {
    ...
    try {
      ...
      setError(e?.message || 'Biometrics failed.');
    } finally {
      setIsUpdating(false);
    }
  };
        ...
        <div className="space-y-6">
          ...
          <div className="pt-4">
            <button
              ...
              disabled={isUpdating}
              className="w-full bg-white/5 border border-white/10 text-slate-200 py-4 rounded-xl font-semibold text-sm hover:bg-white/10 transition-all flex items-center justify-center gap-2 disabled:opacity-60"
              type="button"
            >
              <span className="text-sky"><Fingerprint size={14} /></span>
              {biometricsOn ? 'Disable biometrics' : 'Enable biometrics'}
            </button>
            <p className="mt-2 text-[12px] text-slate-500 leading-snug">
              Uses your device biometrics (passkey) to unlock the app on this device.
            </p>
          </div>
...
const MetricCard = ({ label, value, icon, currency, trend, color, dark, onClick }: any) => (
  <button
    type="button"
    onClick={onClick}
    className={cn(
    ...
    onClick ? "cursor-pointer hover:bg-white/5 active:scale-[0.99] transition-transform" : "cursor-default",
    )}
  >
    <div className="flex items-center justify-between relative z-10">
      <p className={cn("text-[12px]", color === 'danger' ? "text-danger" : "text-slate-400")}>{label}</p>
